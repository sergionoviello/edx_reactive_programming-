...
object SelectiveReceive {
  ...
  def apply[T: ClassTag](bufferCapacity: Int, initialBehavior: Behavior[T]): Behavior[T] =
    Behaviors.withStash(bufferCapacity)(stash =>
      intercept(bufferCapacity, stash, Behavior.validateAsInitial(initialBehavior))
    )
  ...
  private def intercept[T: ClassTag](bufferSize: Int, buffer: StashBuffer[T], started: Behavior[T]): Behavior[T] =
    Behaviors.receive {
      case (ctx, message) =>
        val beh = Behavior.interpretMessage(started, ctx, message)
        if (Behavior.isUnhandled(beh)) {
          buffer.stash(message)
          Behaviors.same
        } else {
          val next = Behavior.canonicalize(beh, started, ctx)
          buffer.unstashAll(SelectiveReceive(bufferSize, next))
        }
