...
object Transactor {
  ...
  private def idle[T](value: T, sessionTimeout: FiniteDuration): Behavior[PrivateCommand[T]] =
    Behaviors receivePartial {
      case (ctx, Begin(replyTo)) =>
        ...
        replyTo ! session
        next
      case (ctx, Committed(_, _)) => Behaviors.ignore
      case (ctx, RolledBack(_)) => Behaviors.ignore
  ...
  private def sessionHandler[T](currentValue: T, commit: ActorRef[Committed[T]], done: Set[Long]): Behavior[Session[T]] =
    Behaviors receive {
      ...
      case (ctx, Rollback()) =>
        Behaviors.stopped
      case (ctx, Modify(f, id, reply, replyTo)) if done.isEmpty || id > done.max =>
        try {
      ...
      case (ctx, Modify(f, id, reply, replyTo)) if  id <= done.max =>
        replyTo ! reply
        Behaviors.same
