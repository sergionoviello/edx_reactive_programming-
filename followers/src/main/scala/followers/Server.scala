...
object Server extends ServerModuleInterface {
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      ...
      def findInBuffer(acc: List[Event], seq: Int): List[Event] = {
        if (buffer get seq isEmpty) acc
        else if (buffer get seq nonEmpty) {
          expectedSeqNr += 1
          (buffer(seq) :: Nil) ++ acc
        }
        else {
          findInBuffer(acc, seq + 1)
        }
      }

      element => {
        val seqNr = element.sequenceNr
        if (seqNr == expectedSeqNr) {
          expectedSeqNr += 1

          element :: findInBuffer(List[Event](), expectedSeqNr)
        } else {
          buffer += seqNr -> element
          Nil
        }
      }
    }
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      var followers = Map[Int, Set[Int]]()

      {
        case ev @ Follow(seq, from, to) =>
          val following = followers.getOrElse(from, Set())
          followers += from -> (following + to)

          List((ev, followers))
        case ev @ Unfollow(seq, from, to) =>
          if (followers get from nonEmpty) {
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean = {
    (userId, eventAndFollowers) match {
      case (uid, (Broadcast(_), followers)) =>
        true
      case (uid, (Follow(_, _, toUserId), followers)) =>
        userId == toUserId
      case (uid, (PrivateMsg(_, _, toUserId), followers)) =>
        userId == toUserId
      case (uid, (StatusUpdate(_, fromUserId), followers)) =>
        followers.getOrElse(userId, Set[Int]()).contains(fromUserId)
    }
  }
...
class Server()(implicit executionContext: ExecutionContext, materializer: Materializer)
  ...
  def outgoingFlow(userId: Int): Source[ByteString, NotUsed] =
    broadcastOut.filter(isNotified(userId)).map {
      case (event, followers: Followers) => event.render
    }
...
//    clientIdPromise.future.map(id => actorSystem.log.info("Connected follower: {}", id.userId))
    ...
    val incoming: Sink[ByteString, NotUsed] =
      identityParserSink.mapMaterializedValue { v =>
        clientIdPromise.completeWith(v)
        NotUsed
      }
